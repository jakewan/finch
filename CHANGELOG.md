...
- Every money amount finch records is now checked before it is stored, on all four paths that write one: recording a transaction, making a transfer, creating a recurring rule, and changing a rule's amount. An amount of zero is refused, because it records an event and moves no balance. So is an amount above $1 trillion in magnitude, because a projection sums a recurring rule's occurrences into one running total and an unbounded amount could overflow it. Previously only a transfer's amount was checked, and only for being positive — every other amount an `int64` can hold was accepted. Ordinary amounts are unaffected: an expense is still negative and income still positive, and the limit is far above any amount the app could ever submit. A rejected amount now returns `InvalidArgument` naming the offending field, rather than `Internal`.
- Creating a transfer with a blank name returns `InvalidArgument` rather than `Internal`, and updating the status of a transaction that does not exist returns `NotFound` rather than `Internal` — so callers can tell a bad request from a daemon failure, matching how the recurring-rule operations already report these.
